...
{
public:
    static void PktCallback(u_char*, const struct pcap_pkthdr*, const u_char*);

    static int linkhdrlen_;

    LibpcapSniffer();
    ~LibpcapSniffer();

    pcap_t* OpenPcapSocket(char* device);
    void CaptureLoop(pcap_t*);
    void SetMaxNumOfPacket(int);
    void SetPacketLenght(int);
    void SetTimeout(int);
